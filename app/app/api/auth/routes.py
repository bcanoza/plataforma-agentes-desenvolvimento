...
"""
Controller unificado de autenticação.
Combina funcionalidades de auth_controller.py e auth_ui_controller.py
"""
...
from fastapi import APIRouter, Request, Response, Body, Cookie, Header
...
from .schemas import (
    ...
    ErrorResponse, ApiKeyValidationResponse, User, build_user_response
)
from .dependencies import verify_api_key, require_user, CurrentUser

logger = get_logger("auth")

# Router principal de autenticação
...
#   HELPERS
# =========================
def _error_response(status_code: int, *, error: str, message: str, details: Optional[dict] = None,
...
def _client_hints(req: Request) -> tuple[Optional[str], Optional[str]]:
    """Captura User-Agent e IP do cliente."""
...
async def login(
    ...
    """Login de usuário - retorna JWT tokens."""
    ...
    try:
        ...
        # Seta cookie HttpOnly + Secure com o refresh
...
async def refresh(
    ...
    try:
        ...
        # Renova o cookie com o NOVO refresh
        if bundle.refresh_token:
            response.set_cookie(
                key="refreshToken",
                value=bundle.refresh_token,
...
async def logout(
    ...
    """Logout de usuário - invalida refresh token."""
...
def validate_api_key(
    ...
    """Valida API Key para comunicação entre microserviços."""
...
def jwks():
    """Endpoint JWKS para validação de JWT."""
    from app.controllers.jwk_controller import jwks as jwks_handler
    return jwks_handler()
